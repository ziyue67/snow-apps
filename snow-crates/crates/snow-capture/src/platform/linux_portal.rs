...
unsafe extern "C" {
    ...
    fn xdp_portal_create_screencast_session(
        portal: *mut XdpPortal,
        outputs: c_int,
        flags: c_int,
        cursor_mode: c_int,
        persist_mode: c_int,
        restore_token: *const c_char,
        cancellable: *mut c_void,
        callback: GAsyncReadyCallback,
        data: *mut c_void,
    );
    fn xdp_portal_create_screencast_session_finish(
        portal: *mut XdpPortal,
        result: *mut c_void,
        error: *mut *mut GError,
    ) -> *mut XdpSession;
    fn xdp_session_start(
        session: *mut XdpSession,
        parent: *mut c_void,
        cancellable: *mut c_void,
        callback: GAsyncReadyCallback,
        data: *mut c_void,
    );
    fn xdp_session_start_finish(
        session: *mut XdpSession,
        result: *mut c_void,
        error: *mut *mut GError,
    ) -> c_int;
...
pub(crate) fn open_portal_stream() -> anyhow::Result<c_int> {
    unsafe {
        ...
        let loop_ = g_main_loop_new(std::ptr::null_mut(), 0);
        ...
        let session =
            xdp_portal_create_screencast_session_finish(portal, state.result, &mut error);
        if session.is_null() {
        ...
        if xdp_session_start_finish(session, state.result, &mut error) == 0 {
